...
def Normal(x, m, s, w, y=None): #probability that, given mean m and std s, that a randomly selected individual in a population is x
    z = (x - m) / s
    if w == 'p':
        return (1 / (np.sqrt(2 * np.pi) * s)) * np.exp(-0.5 * ((x - m) / s) ** 2)
    elif w == 'c':
        return 0.5 * (1 + scipy.special.erf(z * (1 / np.sqrt(2))))
    elif w == 'cb' and y is not None: #cumulative between
...
def exp_dist(x, rate, w): 
    ...
    if w == 'p': #likelihood of waiting x time for an event, given rate per unit of time
...
def find_intersections(dist1, params1, dist2, params2, x_vals):
    ...
    for i in range(len(x_vals) - 1):
        ...
        try:
            if cdf_difference(x1) * cdf_difference(x2) < 0:
...
# Use this function to compute quantiles as well
def quantile(dist, params, x):
    if dist == 'Normal':
        m, s = params[1], params[2]
        return Normal_quantile(x, m, s)
    elif dist == 'Uniform Continuous':
        ...
        values, probs = params
        return custom_quantile(x, values, probs)
    else:
        raise ValueError("Quantile: Unsupported distribution.")
...
def calculate():
    try:
        ...
        if w == 'CDF Comparison':
            ...
            intersections = find_intersections(dist1, dist1_params, dist2, dist2_params, np.linspace(0, 1, 1000))
            print(intersections)
            intercepts_label.config(text=f'Intersections: {intersections}')
